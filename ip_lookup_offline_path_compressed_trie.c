...
#include "path_compressed_trie.h"
...
struct BtNode                   *bt_node_pre;       /* pointer to the previous node of the binary tree */
struct PctNode                  *pct_root;       /* pointer to the root node of the path compressed tree */
...
int main(int argc, char **argv)
{
	...
	//dbt_root = trans_bt2dbt(bt_root);
	trans_bt2dbt(bt_root);

	/* transform to path compressed trie*/
    pct_root = init_pctnode();
	bt_node_pre = init_btnode();
	trans_dbt2pct(pct_root, bt_node_pre, bt_root);
